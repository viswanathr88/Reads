...
namespace Epiphany.Xml.Tests
{
    ...
    public class ParseTests
    {
        ...
        [TestMethod]
        public async Task AuthenticatedUserParseTest()
        {
            url = "Input\\AuthUser.xml";
            string xml = await ReadFile(url);
            Parser.GetAuthUserResponse(xml);
            var actual = Parser.GetAuthUserResponse(xml);
            var expected = await GetAuthUserResponse(url);

            CompareLogic logic = new CompareLogic();
            ComparisonResult result = logic.Compare(actual.AuthenticatedUser, expected.AuthenticatedUser);
        ...
        public async Task BooksInShelfParseTest()
        {
            ...
            CompareLogic logic = new CompareLogic();
            ComparisonResult result = logic.Compare(actual.Reviews, expected.Reviews);
            Assert.IsTrue(result.AreEqual, result.DifferencesString);
        }
        ...
        private async Task<AuthUserResponse> GetAuthUserResponse(string url)
        {
            var folder = Package.Current.InstalledLocation;

            using (Stream stream = await folder.OpenStreamForReadAsync(url))
            {
                return TestParser.ParseUserXml(stream);
            }
        }
    }
}
